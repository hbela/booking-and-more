...
describe.skipIf(!databaseUrl)("conversational booking", () => {
  ...
  it("records the channel a voice conversation booked through", async () => {
    const site = await clinic("spoken");
    const { conversation, confirmation } = await upToConfirmation(site, "VOICE");
    ...
    expect(booking?.source).toBe("VOICE");
  });
  ...
  it("stops talking once the tenant's allowance is gone", async () => {
    ...
    expect(response.json<{ error: { code: string } }>().error.code).toBe("USAGE_QUOTA_EXCEEDED");
  });
});
