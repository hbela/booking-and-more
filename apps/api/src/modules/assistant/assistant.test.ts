...
describe("company profile translations", () => {
  ...
  it("falls back to the original language for an absent or empty translation", () => {
    expect(localizedBusinessDescription({ ...profile, businessDescriptionEn: null }, "en", "hu"))
      .toBe(profile.businessDescriptionHu);
    expect(localizedBusinessDescription({ ...profile, businessDescriptionHu: "" }, "hu", "en"))
      .toBe(profile.businessDescriptionEn);
  });

  it("preserves legacy profiles and handles missing settings", () => {
    expect(localizedBusinessDescription({
      businessDescription: "Legacy profile", businessDescriptionHu: null, businessDescriptionEn: null,
      businessDescriptionDe: null, businessDescriptionFr: null,
    }, "en", "hu")).toBe("Legacy profile");
  ...
  it.each(["de", "fr"] as const)("accepts %s FAQs and all four assistant languages", (locale) => {
    expect(assistantFaqInputSchema.parse({ locale, question: "Question", answer: "Answer" }).locale)
      .toBe(locale);
    expect(assistantSettingsPatchSchema.parse({ supportedLocales: ["hu", "en", "de", "fr"] }).supportedLocales)
      .toEqual(["hu", "en", "de", "fr"]);
  });

  it.each([
    ["de", "businessDescriptionDe"], ["fr", "businessDescriptionFr"],
  ] as const)("uses %s as the original profile and preserves legacy writes", async (locale, field) => {
    expect(localizedBusinessDescription({ ...profile, businessDescriptionHu: null }, "hu", locale))
      .toBe(profile[field]);
    const upsert = vi.fn().mockResolvedValue(profile);
    const prisma = { tenantAssistantSettings: { upsert } } as unknown as PrismaClient;
    await new AssistantService(prisma).saveSettings("tenant-1", { businessDescription: "Updated" }, locale);
    expect(upsert).toHaveBeenCalledWith(expect.objectContaining({
      update: { businessDescription: "Updated", [field]: "Updated" },
    }));
    expect(assistantSettingsPatchSchema.safeParse({ [field]: "x".repeat(4001) }).success).toBe(false);
  });

  it("validates and trims each language independently", () => {
    expect(assistantSettingsPatchSchema.parse({ businessDescriptionEn: " English " }))
      .toEqual({ businessDescriptionEn: "English" });
    expect(assistantSettingsPatchSchema.safeParse({ businessDescriptionHu: "x".repeat(4001) }).success)
      .toBe(false);
    expect(assistantSettingsPatchSchema.parse({ businessDescriptionHu: null }))
      .toEqual({ businessDescriptionHu: null });
  });
  ...
  it("keeps the other language unchanged when saving a translation", async () => {
    const upsert = vi.fn().mockResolvedValue(profile);
    const prisma = { tenantAssistantSettings: { upsert } } as unknown as PrismaClient;
    await new AssistantService(prisma).saveSettings("tenant-1", { businessDescriptionEn: "New English" }, "hu");
    expect(upsert).toHaveBeenCalledWith(expect.objectContaining({
      where: { tenantId: "tenant-1" }, update: { businessDescriptionEn: "New English" },
    }));
  });
  ...
  it("clears the legacy fallback when the original profile is cleared", async () => {
    const upsert = vi.fn().mockResolvedValue(profile);
    const prisma = { tenantAssistantSettings: { upsert } } as unknown as PrismaClient;
    await new AssistantService(prisma).saveSettings("tenant-1", { businessDescriptionHu: null }, "hu");
    expect(upsert).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("saves four languages and supplies matching profiles and FAQs to chat", async () => {
    ...
    try {
      ...
      await prisma.location.create({ data: {
        tenantId: tenant.id, name: "Main office", timezone: "Europe/Budapest",
        addressLine1: "Example street 12", city: "Test City", postalCode: "1234",
      } });
      await service.saveSettings(tenant.id, { businessDescriptionHu: profile.businessDescriptionHu }, "hu");
      await service.saveSettings(tenant.id, { businessDescriptionEn: profile.businessDescriptionEn }, "hu");
      await service.saveSettings(tenant.id, {
      ...
      for (const [locale, field] of [
        ...
        await service.createFaq(tenant.id, {
          locale, question: `${locale} question`, answer: `${locale} approved answer`, active: true, sortOrder: 0,
        });
        ...
        expect(await service.knowledgeContext(tenant, "hu")).not.toContain(`${locale} approved answer`);
      }
