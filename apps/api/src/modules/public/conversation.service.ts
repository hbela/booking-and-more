...
import type { ValidatedAudio } from "./audio.js";
...
export class ConversationService {
  ...
  async message(args: {
    ...
    await this.repo.appendMessage({
      ...
      conversationId: session.id,
      sender: "CUSTOMER",
      messageType: args.spoken ? "VOICE" : "TEXT",
      content: args.text,
    });

    // Before the model, never after. A check that runs afterwards is an
    // accounting entry, not a limit.
    await this.usage.assertAllowed({
      tenantId: tenant.id,
      category: "AI_INPUT_TOKENS",
      requestedQuantity: ESTIMATED_INPUT_TOKENS,
      now: args.now,
    });
    ...
    const interpretation = await this.providers.interpreter.interpret({
      utterance: args.text,
      locale: session.locale,
      timezone: session.timezone,
      state,
    ...
    await this.recordInterpretationUsage(tenant.id, interpretation.usage, args.now);
      ...
      const { value } = await withIdempotency(
        ...
        async () => {
          const created = await this.bookings.confirmBooking({
            ...
            now: args.now,
            // The first code path in the product to set these. The enum has
            // existed unused since the booking engine landed.
            source: session.channel === "VOICE" ? "VOICE" : "CHAT",
  ...
  private async recordInterpretationUsage(
    tenantId: string,
    usage: { provider: string; model: string; inputTokens?: number; outputTokens?: number; estimatedCostMinor: number },
    now: Date,
  ): Promise<void> {
    // Cost is attributed to the input side; splitting it across two rows would
    // double-count it in any total that sums `estimated_cost_minor`.
    await this.usage.record(
      {
        tenantId,
        category: "AI_INPUT_TOKENS",
