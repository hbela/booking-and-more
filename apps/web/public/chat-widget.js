(() => {
  ...
  button.type = "button";
  button.textContent = locale === "hu" ? "Segíthetek?" : "Can I help?";
  ...
  window.addEventListener("message", (event) => {
    if (
      ...
      return;
    if (event.data.type === "bam-chat:close") {
      frame.style.display = "none";
      button.setAttribute("aria-expanded", "false");
    }
    if (event.data.type === "bam-chat:resize" && Number.isFinite(event.data.height))
      frame.style.height = `${Math.max(320, Math.min(680, event.data.height))}px`;
  });
