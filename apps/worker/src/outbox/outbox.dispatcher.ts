...
async function dispatchProviderEvent(
  ...
  if (event.eventType === "ASSISTANT_INVITED") {
    return dispatchAssistantInvited(event, options);
  }
...
/**
 * Somebody invited to assist on one diary.
 * docs/phase-3-4-diary-delegation.md §2.13.
 *
 * The sibling of `dispatchProviderEvent`'s own branch, and deliberately a
 * separate function rather than a flag on it: the two emails address different
 * people about different jobs, and the one thing this must not do is greet the
 * recipient as the provider whose diary they are being given.
 *
 * The dedupe key is the event id, so a re-invite legitimately earns a second
 * email — a re-invite means the first did not arrive.
 */
async function dispatchAssistantInvited(
  event: ClaimedOutboxEvent,
  options: DispatcherOptions,
): Promise<DispatchOneResult> {
  const payload = event.payload as {
  ...
  // A cleared payload means this event was already dispatched and re-claimed
  // before the status write — correctly a no-op, since the notification exists.
  if (recipient === undefined || recipient === "" || token === undefined) {
    options.logger.warn(
      { eventId: event.id },
      "outbox: assistant invitation has no usable payload; already dispatched",
    );
    return { created: 0, duplicate: 0 };
  }
  ...
  if (!tenant) {
    options.logger.warn({ eventId: event.id }, "outbox: tenant is gone; dropping invitation");
    return { created: 0, duplicate: 0 };
  }
  ...
  // Read from the invitation rather than the payload: the scopes are what the
  // acceptance will actually apply, and an email that promised a different set
  // would be a lie the recipient could not check.
  const invitation = await options.prisma.invitation.findFirst({
    where: {
      tenantId: event.tenantId,
      email: recipient,
      delegatedProviderId: event.aggregateId,
      status: "PENDING",
    },
  ...
  const created = await persistNotification(options.prisma, event.tenantId, {
    type: NotificationTypes.ASSISTANT_INVITED,
    channel: NotificationChannels.EMAIL,
    template: "assistant-invited",
    // The organization's language: nothing is known about this person yet.
    locale: resolveLocale({ tenantLanguage: tenant.defaultLanguage }),
    recipient,
    scheduledAtIso,
    dedupeKey: buildDedupeKey({
    ...
    payload: {
      organizationName: tenant.name,
      providerName: provider?.displayName ?? payload?.providerName ?? "",
      invitedByName: payload?.invitedByName ?? null,
