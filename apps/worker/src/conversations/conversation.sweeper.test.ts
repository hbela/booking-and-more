...
describe.skipIf(!databaseUrl)("conversation sweeper", () => {
  ...
  it("erases an old transcript and keeps the row that priced it", async () => {
    ...
    const old = await prisma.conversationMessage.create({
      data: {
        tenantId,
        sessionId: session.id,
        sender: "CUSTOMER",
    ...
    const recent = await prisma.conversationMessage.create({
      data: {
        tenantId,
        sessionId: session.id,
        sender: "CUSTOMER",
    ...
    const kept = await prisma.conversationMessage.findUniqueOrThrow({ where: { id: recent.id } });
    expect(kept.content).toBe("recent message");
  });
});
