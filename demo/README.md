...
These independent static sites replace `customer-site`. Hungarian is served at `/`, English at `/en/`. All booking, AI Assistant and staff-login links open `https://app.booking.appointer.hu` using an explicit tenant slug. The referring website domain is never used to select a tenant. No secrets or environment variables are required by these sites.

## Deployment order

1. Deploy this revision to the **existing staging application resource** first. Keep its database, Redis volumes and environment settings intact. This change adds no migration or backend endpoint.
2. Verify `/wellness/sign-in`, `/medicare/sign-in`, `/en/wellness/sign-in` and `/en/medicare/sign-in` over HTTPS. Shared `/sign-in` remains available; platform administrators continue to land on `/admin`.
...
| Domain                         | `https://wellness.appointer.hu`   | `https://medicare.appointer.hu`   |
...
Create DNS A records `wellness` and `medicare` under `appointer.hu`, both pointing to the VPS public IPv4 address. Add AAAA records only if IPv6 routing is configured on that VPS. Configure both HTTPS domains in Coolify and verify certificate issuance and HTTP-to-HTTPS redirects.
...
- Follow all three links on each language version. Hungarian links must use `/{slug}/book`, `/{slug}/chat`, `/{slug}/sign-in`; English links must use `/en/{slug}/…` on the staging app. The bare app paths `/wellness` and `/medicare` still redirect to booking.
...
- Both demo HTTPS domains respond with HTTP 200, but this does not verify that they serve these new files. `/medicare/sign-in` on the staging app still returns 404, so the new application route is not deployed there.
