...
1. **Nothing sends yet.** The `notifications` queue has no consumer until part 2. Running the worker now
   ...
   **Overtaken by Epic 9**, which needed email before this phase's part 2 arrived and therefore built the
   ...
   `PROVIDER_INVITED` joined them in [phase-9-provider-onboarding.md](phase-9-provider-onboarding.md).

   **Closed on 2026-08-05** by [phase-5-booking-notifications.md](phase-5-booking-notifications.md), which
   added the five booking templates this phase was scoped around, plus a seventh type
